 ...
 * @date 24.12.2019
 */
public class UpdateTrainController implements Initializable
{
    ...
    private void handleAdd(ActionEvent event)
    {
        try
        {
            ...
            boolean updated = operation.updateTrain(trainidnumber, traindpt, trainart, date, traincost, traindpc, trainarc, trainarc);
            if(updated)
            {
                if(middlecity.equals("") || middlecity.equals(null))
                {
                    for(int i = 0; i < 20; i++)
                    {
                        operation.updateSeatTrain(trainidnumber, null, trainarc, String.valueOf(i + 1), true);
                    }
                }
                else
                {
                    for(int i = 0; i < 20; i++)
                    {
                        operation.updateSeatTrain(trainidnumber, middlecity, trainarc, String.valueOf(i + 1), true);
                    }
                }
                lbl_situtation.setText("Train Updated!");
            }
        }
